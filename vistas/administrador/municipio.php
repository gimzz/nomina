<?php
  ...
  if(isset($_GET['deleteId']) && !empty($_GET['deleteId'])) {
      $deleteId = $_GET['deleteId'];
       $resultado = $persona->delete_municipio($deleteId);
       if ($resultado===false) {
...
<div class="container">
  <br>
  <h2> Registro de Municipios
    <a href="crear_municipio.php" class="btn btn-primary" style="float:right;">Añadir Municipio</a>
  </h2>
...
if (!empty($deliverys)) {
    foreach ($deliverys as $delivery) {
        ...
        <tr>
          ...
          <td><?php echo $delivery['nombre_municipio'] ?></td>
          <td><?php echo $delivery['nombre_estado'] ?></td>
          <td><?php echo $delivery['estatus'] ?></td>
          <td><a href="editar_municipio.php?editId=<?php echo $delivery['id_municipio'] ?>" style="color:green"><i class="fa fa-pencil" aria-hidden="true"></i></a>
          <a href="#" onclick="confirmar('<?php echo $delivery['id_municipio'] ?>')" style="color:red">
    ...
    function confirmar(cedula) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = "municipio.php?deleteId=" + cedula;
            }
        });
    }
</script>
